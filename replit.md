...
- July 21, 2025. Intelligent CPF formatting and consultation system improvements: Implemented smart CPF formatting that only formats on blur/submit instead of every keystroke (preventing 000.000.000-01 issue), added real-time CPF validation with visual feedback, created BenefitSelector component for streamlined benefit selection after CPF search, optimized consultation saving to store only formatted CPF instead of both CPF and benefit number, enhanced ConsultationHistory component with proper formatting and CSV export functionality, improved user experience with cleaner interface and better data handling