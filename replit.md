...
- July 30, 2025. Enhanced Banco Safra simulation with customizable options: Added user-selectable prazo (term) options (96,90,84,72 months), implemented optional parcela (installment value) input for value-based simulations, added toggle between prazo and parcela simulation modes, maintained multi-contract selection capability, standardized all simulation buttons to blue color scheme for consistency across all banks, updated to show all Safra contracts from client instead of single contract, removed "Prosseguir com o refinanciamento" section for simulation-only functionality, implemented accurate parcelas pagas calculation using (Prazo Total - Parcelas Restantes) formula
- July 30, 2025. Standardized all banking action buttons across the system: Unified all "Ações" buttons for Banrisul (Simular with Calculator icon), C6 Bank (Digitar with FileText icon), and Banco Safra (Simular with BarChart3 icon) to use identical blue professional styling (bg-blue-600 hover:bg-blue-700), consistent dimensions (min-w-[80px] h-8), professional animations with hover effects and active scale, comprehensive dark mode support, ensuring uniform user experience across all three banking simulation interfaces
- July 30, 2025. Enhanced Banco Safra simulation with complete term range and intelligent filtering: Implemented full term selection from 60-96 months (37 individual options), added smart default behavior where empty selection shows only the highest available term from API response, created precise filtering where selected terms show only those specific results, improved error messages to display direct API criticisms with "Crítica da API:" prefix, maintained all existing functionality with multi-contract selection and parcelas pagas calculation
- July 30, 2025. Enhanced phone number display with WhatsApp integration and copy functionality: Added WhatsApp icon buttons that open https://wa.me/55[phone] links (55 is fixed prefix + client phone without punctuation), implemented minimalist copy button with check confirmation feedback, redesigned phone number display with separate clickable areas for WhatsApp and copy actions, applied consistent styling across both benefit details and consultation history components for unified user experience
- July 30, 2025. Updated phone copy functionality to use formatted format: Modified copy buttons to copy formatted phone numbers ((dd)nnnnn-nnnn) instead of raw digits, ensuring users get properly formatted Brazilian phone numbers when copying to clipboard, maintained WhatsApp functionality using clean digits for URL compatibility, removed extra spaces for cleaner formatting
- July 30, 2025. Enhanced C6 Bank proposal report completeness: Fixed gross_amount calculation to use (installment_amount * installment_quantity) formula, added comprehensive sections including Origins/Correspondent, Agreement/Product, Formalization Info, expanded tax details with all rates (appropriation, nominal), added Loan Track history section, implemented console logging of complete proposal data, created summary section with JSON preview, improved visual hierarchy with section-specific colors and icons
- July 30, 2025. Fixed C6 Bank digitization history display: Corrected "Valor Bruto" calculation to use (installment_amount × installment_quantity) formula matching proposal report, added "Valor Solicitado" field for clarity, enhanced parcela display to show quantity (e.g., R$ 32.19 x 96), implemented table used display (INSS Refin 1/2/3 - Web) extracted from creditCondition data, ensuring consistency between digitization history and proposal reports